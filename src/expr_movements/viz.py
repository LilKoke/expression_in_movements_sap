...
def feature_pca(features_df, out_path: str | Path, *, title: str | None = None) -> Path:
    """PCA-scatter expert features, coloured by emotion and marker-shaped by subject.

    ``features_df`` is the clip-level expert feature table generated by
    ``expr-featurize``. The four expert features are standardised before PCA so
    scale differences between feature units do not dominate the projection.
    """
    ...
    required = {"label", "subject", *EXPERT_FEATURE_COLUMNS}
    missing = required - set(features_df.columns)
    if missing:
        raise ValueError(f"expert feature table missing columns: {sorted(missing)}")

    x = features_df.loc[:, list(EXPERT_FEATURE_COLUMNS)].to_numpy(dtype=np.float32)
    labels = features_df["label"].astype(str).to_numpy()
    subjects = features_df["subject"].astype(str).to_numpy()
    ...
    ax.set_xlabel(f"PC1 ({evr[0] * 100:.1f}% var)")
    ax.set_ylabel(f"PC2 ({evr[1] * 100:.1f}% var)")
    ax.set_title(title or "Expert-feature PCA")
    fig.tight_layout()
